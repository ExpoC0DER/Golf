...
namespace _game.Scripts.ObstacleScripts
{
    ...
    public class BlackHole : MonoBehaviour
    {
        ...
        private void Awake() { _obstacleBase = GetComponent<ObstacleBase>(); }
        ...
        private void FixedUpdate()
        {
            ...
            foreach (Rigidbody player in _obstacleBase.PlayersInRange)
            {
                ...
                float distanceFromCenter = Mathf.Pow(Vector3.Distance(position, playerPosition), 2);
                float scaledStrength = distanceFromCenter.RemapClamped(0, _range.lossyScale.x, _strength, 0);
                player.AddForce((position - playerPosition).normalized * (scaledStrength * Time.fixedDeltaTime), ForceMode.Acceleration);
            }
        }
    }
}
